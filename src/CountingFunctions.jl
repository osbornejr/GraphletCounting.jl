    ...
    for k in gamma_i
        ...
        for w in Tri 
            for v  in neighbourdict[w]
                ...
                ## separating the processes here so that we can maintain the right type ordering 
                elseif (v in iPath) 
                    count_dict[graphlet_string(vertex_type_list[j],vertex_type_list[i],vertex_type_list[w],vertex_type_list[v],"4-chord-edge-orbit",delim)]+=1
                elseif (v in jPath) 
                    count_dict[graphlet_string(vertex_type_list[i],vertex_type_list[j],vertex_type_list[w],vertex_type_list[v],"4-chord-edge-orbit",delim)]+=1
                elseif (!(v in gamma_i) & !(v in gamma_j))
            ...
            for (indb,b) in enumerate(types[inda:end])
                if (a == b)
                    ...
                    if (vertex_type_list[i]!=vertex_type_list[j])
                        ...
                        #i-centre
                        count_dict[graphlet_string(b,vertex_type_list[j],vertex_type_list[i],a,"4-tail-tri-edge-orbit",delim)] += TriTypes[inda]*iPathTypes[inda] -     count_dict[graphlet_string(vertex_type_list[j],vertex_type_list[i],b,a,"4-chord-edge-orbit",delim)]
                        #j-centre
                        count_dict[graphlet_string(b,vertex_type_list[i],vertex_type_list[j],a,"4-tail-tri-edge-orbit",delim)] += TriTypes[inda]*jPathTypes[inda] -     count_dict[graphlet_string(vertex_type_list[i],vertex_type_list[j],b,a,"4-chord-edge-orbit",delim)]
                        ...
                        ##4-tail tri-edge orbit
                        count_dict[graphlet_string(b,vertex_type_list[i],vertex_type_list[j],a,"4-tail-tri-edge-orbit",delim)] += TriTypes[inda]*iPathTypes[inda] + TriTypes[inda]*jPathTypes[inda] - count_dict[graphlet_string(vertex_type_list[j],vertex_type_list[i],b,a,"4-chord-edge-orbit",delim)]
                    end
                    ...
                    ##For the other two graphlets, we operate differently if i and j are of different types
                     if (vertex_type_list[i]!=vertex_type_list[j])
                        ...
                        #i-centre, a tail
                        count_dict[graphlet_string(b,vertex_type_list[j],vertex_type_list[i],a,"4-tail-tri-edge-orbit",delim)] += TriTypes[inda+indb-1]*iPathTypes[inda] - count_dict[graphlet_string(vertex_type_list[j],vertex_type_list[i],b,a,"4-chord-edge-orbit",delim)]
                        #i-centre, b tail
                        count_dict[graphlet_string(a,vertex_type_list[j],vertex_type_list[i],b,"4-tail-tri-edge-orbit",delim)] += TriTypes[inda]*iPathTypes[inda+indb-1] - count_dict[graphlet_string(vertex_type_list[j],vertex_type_list[i],a,b,"4-chord-edge-orbit",delim)]
                        #j-centre, a tail
                        count_dict[graphlet_string(b,vertex_type_list[i],vertex_type_list[j],a,"4-tail-tri-edge-orbit",delim)] += TriTypes[inda+indb-1]*jPathTypes[inda] - count_dict[graphlet_string(vertex_type_list[i],vertex_type_list[j],b,a,"4-chord-edge-orbit",delim)]
                        #j-centre, b tail
                        count_dict[graphlet_string(a,vertex_type_list[i],vertex_type_list[j],b,"4-tail-tri-edge-orbit",delim)] += TriTypes[inda]*jPathTypes[inda+indb-1] - count_dict[graphlet_string(vertex_type_list[i],vertex_type_list[j],a,b,"4-chord-edge-orbit",delim)]
                        ...
                        #a tail
                        count_dict[graphlet_string(b,vertex_type_list[i],vertex_type_list[j],a,"4-tail-tri-edge-orbit",delim)] += TriTypes[inda+indb-1]*iPathTypes[inda] + TriTypes[inda+indb-1]*jPathTypes[inda] - count_dict[graphlet_string(vertex_type_list[i],vertex_type_list[j],b,a,"4-chord-edge-orbit",delim)]
                        #j-centre, b tail
                        count_dict[graphlet_string(a,vertex_type_list[i],vertex_type_list[j],b,"4-tail-tri-edge-orbit",delim)] += TriTypes[inda]*iPathTypes[inda+indb-1] + TriTypes[inda]*jPathTypes[inda+indb-1] - count_dict[graphlet_string(vertex_type_list[i],vertex_type_list[j],a,b,"4-chord-edge-orbit",delim)]
                    end
                end
            end
        end
